#!/usr/bin/fish
set dotfiles_dir "$HOME/jasmith79-dotfiles"
...
if command -v node >/dev/null
    ...
    set MEETS_REQ "$(bash $dotfiles_dir/utils/check-version "v22.21.1" "$CURRENT_NODE_VERS")"
    if [ "$MEETS_REQ" != passed ]
        and [ -x "$(command -v fnm)" ]
        fnm use --install-if-missing 22.21 2>&1 >/dev/null
    end
end
